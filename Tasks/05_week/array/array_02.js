...
let filteredAnimals = reserve1Animals.filter((animal) =>
  reserve2Animals.includes(animal)
);
filteredAnimals.forEach((species) => {
  console.log(`Common species: ${species}`);
});
...
animalNames.forEach((name) => {
  ...
  console.log(`Letters in "${name}": ${letters.join(", ")}`);
});
...
const getPositiveTemperatures = () => {
  return temperatures.filter((value) => value >= 0);
};
...
if (indexOfPenguin === -1) console.log("Penguin is not in the zoo");
else {
...
console.log("Replaced animals: ", zooAnimals);
...
console.log("Safari animals sorted: ", safariAnimals);
...
console.log("Safari animals reversed: ", safariAnimals);
...
*/
// Write your code here
let hasTiger = safariAnimals.includes("Tiger");
...
attendees.forEach((member, index) => {
  if (member === "John") {
    callJohn.push(index);
  }
});

console.log(callJohn);
...
*/
// Write your code here
let capitalAttendees = attendees.map((name) => name.toUpperCase());
console.log("Names capitalized: ", capitalAttendees);
...
*/
// Write your code here
const merged = [...forestAnimals, ...farmAnimals];
...
birdSpecies.pop();
birdSpecies.shift();
console.log("Removed first and last bird: ", birdSpecies);
...
console.log("Increased prices: ", increasedPrice);
